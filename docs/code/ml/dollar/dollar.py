# $1 Recognizer in Python by Shaun Kane
# Original by Jacob O. Wobbrock, Andrew D. Wilson, Yang Li
...
class Template:
	def __init__(self, name2, points2):
		...
		self.Points = RotateToZero(self.Points)
		self.Points = ScaleToSquare(self.Points, SquareSize)
...
class Recognizer:
	...
	def RecognizeProbs(self, points, probs):
		points = Resample(points, NumPoints)
		points = RotateToZero(points)
		points = ScaleToSquare(points, SquareSize)
		points = TranslateToOrigin(points)
		...
		for temp in self.Templates:		
			d = DistanceAtBestAngle(points, temp, -AngleRange, +AngleRange, AnglePrecision)
			index = int(temp.Name)
			probs[index] = max(probs[index], 1.0 - (d / HalfDiagonal))            

		
	def AddTemplate(self, name, points):
		self.Templates.append(Template(name, points)) # append new template
		num = 0
		for temp in self.Templates:
			if temp.Name == name:
				num = num + 1
	...
	def DeleteUserTemplates(self):
		self.Templates = self.Templates[0:NumTemplates] # clear any beyond the original set
		return NumTemplates
...
def Resample(points, n):
	...
	# somtimes we fall a rounding-error short of adding the last point, so add it
	# if so
