...
\item For graphics output SDL2 development packages are needed as well as cairo development packages.
...
%----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\begin{problem}{0}{0}{Prepare Raspberrian}


\begin{enumerate}
	\item install correct version of gcc:
	\verb|sudo ap gcc-4.8|
	\item set it as default:
	\verb|sudo ln -s /usr/bin/gcc-4.8 /usr/bin/gcc|
	\item install correct version of portaudio:
	\verb|sudo apt-get install portaudio19-dev|
	\item install cairo:
	\verb|sudo apt-get install libcairo-dev|
	\item installt prebuild sdl-2 packages(in \verb|packages/raspberrian/sdl2/|):
	\verb|sudo dpkg -i *.deb|
