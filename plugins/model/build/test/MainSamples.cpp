...
bool ex_undersample(void *args) {
	...
	ModelTools::PlotSamples(smerge, "original", ssi_rect(650, 0, 400, 400));
	...
	ModelTools::PlotSamples(sunder, "ISUnderSample::RANDOM", ssi_rect(650, 0, 400, 400));
	...
	ModelTools::PrintInfo (sunder);
	ModelTools::PlotSamples(sunder, "ISUnderSample::KMEANS", ssi_rect(650,0,400,400));
...
bool ex_oversample(void *args) {
	...
	ModelTools::PlotSamples(sover, "ISOverSample::SIMPLE", ssi_rect(650, 0, 400, 400));
	...
	ModelTools::PrintInfo (sover);
	ModelTools::PlotSamples(sover, "ISOverSample::SMOTE", ssi_rect(650, 0, 400, 400));
...
bool ex_missingstrm(void *args) {
	...
	// strain
	{
		...
		for (ssi_size_t n_stream = 0; n_stream < n_streams; n_stream++) {
			models[n_stream] = ssi_create(SimpleKNN, 0, true);
		}
...
bool ex_norm(void *args) {
...

	{
		...
		ISNorm::SaveParams("norm_none", params[0], type);
		ISNorm::ZeroParams(params[1], ISNorm::METHOD::SCALE);
		snorm.setNorm(0, params[1]);
		ISNorm::SaveParams("norm_scale", params[1], type);
		ISNorm::ZeroParams(params[2], ISNorm::METHOD::SCALE);
		params[2].limits[0] = -1.0f;
		params[2].limits[1] = 1.0f;
		snorm.setNorm(0, params[2]);
		ISNorm::SaveParams("norm_scale_2", params[2], type);
		ISNorm::ZeroParams(params[3], ISNorm::METHOD::ZSCORE);
		snorm.setNorm(0, params[3]);
		ISNorm::SaveParams("norm_zscore", params[3], type);
	...
	ISNorm::Params params[4];
	ISNorm::LoadParams("norm_none", params[0]);
	ISNorm::LoadParams("norm_scale", params[1]);
	ISNorm::LoadParams("norm_scale_2", params[2]);
	ISNorm::LoadParams("norm_zscore", params[3]);
	...
	ModelTools::SaveSampleList(snorm, "norm", File::ASCII);

	ModelTools::PlotSamples(samples, "RAW", ssi_rect(650, 0, 400, 400));
	ModelTools::PlotSamples(snorm, "NORM", ssi_rect(650, 0, 400, 400));
