...
bool ex_model(void *arg) {
	...
	// evaluation
	{
		...
		ModelTools::PlotSamples(stest, "svm (internal normalization)");
	}
	...
	// evaluation
	{
		...
		trainer.cluster (stest);
		ModelTools::PlotSamples(stest, "knn");
	}
	...
	// evaluation
	{
		...
		trainer.cluster (stest);
		ModelTools::PlotSamples(stest, "bayes");
	}
	...
	// evaluation
	{
		...
		trainer.cluster (stest);
		ModelTools::PlotSamples(stest, "lda");
	}
...
bool ex_hierarchical(void *arg) {
	...
	ModelTools::PlotSamples(strain, "train");
	...
	// non-hierarchical
	{
		...
		trainer.eval(sdevel);
		trainer.cluster(stest);
		ModelTools::PlotSamples(stest, "svm");
...

	{
		...
		trainer.cluster(stest);
		ModelTools::PlotSamples(stest, "hierarchical svm");
	}
...
bool ex_fusion(void *arg) {
	...
	// strain
	{
		...
		for (ssi_size_t n_stream = 0; n_stream < n_streams; n_stream++) {
			...
			models[n_stream] = ssi_create (IModel, string, true);
		}
