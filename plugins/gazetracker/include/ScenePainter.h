...
#ifndef SSI_SCENEPAINTER_H
#define SSI_SCENEPAINTER_H
...
namespace ssi {
	...
	class ScenePainter : public IFilter {
...
public:
	...
	public:
		...
		Options()
			: isFlipped(true), drawHeatmap(false), heatMapPoints(30), printTimes(false) {

			addOption("isFlipped", &isFlipped, 1, SSI_BOOL, "set this to true if the painted detection points are upside down");
			addOption("drawHeatmap", &drawHeatmap, 1, SSI_BOOL, "draws a heatmap in addition to the standard crosshair (pretty, but CPU-intensive. Only use in release build)");
			addOption("heatMapPoints", &heatMapPoints, 1, SSI_INT, "number of gazepoints on the heatmap");
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ScenePainter"; };
	static IObject *Create (const ssi_char_t *file) { return new ScenePainter (file); };
	~ScenePainter();
	ScenePainter::Options *getOptions() { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Paints gazepoint on a scene"; };
...
protected:
	...
	ScenePainter(const ssi_char_t *file = 0);
	ScenePainter::Options _options;
	ssi_char_t *_file;
