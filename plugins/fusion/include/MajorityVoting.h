...
#ifndef SSI_MODEL_MAJORITYVOTING_H
#define SSI_MODEL_MAJORITYVOTING_H
...
class MajorityVoting : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "MajorityVoting"; };
	static IObject *Create (const ssi_char_t *file) { return new MajorityVoting (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Majority Voting"; };
...
protected:
	...
	MajorityVoting (const ssi_char_t *file = 0);
