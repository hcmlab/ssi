...
#ifndef SSI_MODEL_GRADING_H
#define SSI_MODEL_GRADING_H
...
class Grading : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return (samples.getStreamSize ());};
	ssi_size_t getMetaModelNumber(ISamples &samples) { return (samples.getStreamSize ()); };

	static const ssi_char_t *GetCreateName () { return "Grading"; };
	static IObject *Create (const ssi_char_t *file) { return new Grading (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Grading"; };
...
protected:
	...
	Grading (const ssi_char_t *file = 0);
	virtual ~Grading ();
