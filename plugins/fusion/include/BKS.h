...
#ifndef SSI_MODEL_BKS_H
#define SSI_MODEL_BKS_H
...
class BKS : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize (); };

	static const ssi_char_t *GetCreateName () { return "BKS"; };
	static IObject *Create (const ssi_char_t *file) { return new BKS (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Behaviour Knowledge Space (BKS)"; };
	...
	bool checkEntry(ssi_size_t *_table_entry, ssi_size_t *_new_entry, ssi_size_t check_index);
