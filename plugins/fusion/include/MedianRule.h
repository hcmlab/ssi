...
class MedianRule : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "MedianRule"; };
	static IObject *Create (const ssi_char_t *file) { return new MedianRule (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Median Rule"; };
...
protected:
	...
	MedianRule (const ssi_char_t *file = 0);
	virtual ~MedianRule ();
	
	ssi_char_t *_file;

	static void sort (ssi_size_t nSize, ssi_real_t *anArray);
