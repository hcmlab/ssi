...
#ifndef SSI_MODEL_DECISIONTEMPLATE_H
#define SSI_MODEL_DECISIONTEMPLATE_H
...
class DecisionTemplate : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "DecisionTemplate"; };
	static IObject *Create (const ssi_char_t *file) { return new DecisionTemplate (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Decision Template"; };
...
protected:
	...
	DecisionTemplate (const ssi_char_t *file = 0);
	virtual ~DecisionTemplate ();
