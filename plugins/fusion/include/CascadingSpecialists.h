...
#ifndef SSI_MODEL_CASCADINGSPECIALISTS_H
#define SSI_MODEL_CASCADINGSPECIALISTS_H
...
class CascadingSpecialists : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize(); };

	static const ssi_char_t *GetCreateName () { return "CascadingSpecialists"; };
	static IObject *Create (const ssi_char_t *file) { return new CascadingSpecialists (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Cascading Specialists"; };
...
protected:
	...
	CascadingSpecialists (const ssi_char_t *file = 0);
	virtual ~CascadingSpecialists ();
