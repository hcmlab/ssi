...
#ifndef SSI_MODEL_SUMRULE_H
#define SSI_MODEL_SUMRULE_H
...
class SumRule : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "SumRule"; };
	static IObject *Create (const ssi_char_t *file) { return new SumRule (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Sum Rule"; };
...
protected:
	...
	SumRule (const ssi_char_t *file = 0);
	virtual ~SumRule ();
