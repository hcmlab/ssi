...
#ifndef SSI_MODEL_WEIGHTEDAVERAGE_H
#define SSI_MODEL_WEIGHTEDAVERAGE_H
...
class WeightedAverage : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "WeightedAverage"; };
	static IObject *Create (const ssi_char_t *file) { return new WeightedAverage (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Weighted Average"; };
...
protected:
	...
	WeightedAverage (const ssi_char_t *file = 0);
