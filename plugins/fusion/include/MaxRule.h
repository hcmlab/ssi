...
#ifndef SSI_MODEL_MAXRULE_H
#define SSI_MODEL_MAXRULE_H
...
class MaxRule : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "MaxRule"; };
	static IObject *Create (const ssi_char_t *file) { return new MaxRule (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Max Rule"; };
...
protected:
	...
	MaxRule (const ssi_char_t *file = 0);
	virtual ~MaxRule ();
