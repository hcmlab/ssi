...
#ifndef SSI_MODEL_DEMPSTERSHAFER_H
#define SSI_MODEL_DEMPSTERSHAFER_H
...
class DempsterShafer : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "DempsterShafer"; };
	static IObject *Create (const ssi_char_t *file) { return new DempsterShafer (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Dempster Shafer"; };
...
protected:
	...
	DempsterShafer (const ssi_char_t *file = 0);
	virtual ~DempsterShafer ();
	...
	ssi_real_t **_proximities;
	ssi_real_t ***_decision_template;
