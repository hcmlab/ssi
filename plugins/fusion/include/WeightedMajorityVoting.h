...
#ifndef SSI_MODEL_WEIGHTEDMAJORITYVOTING_H
#define SSI_MODEL_WEIGHTEDMAJORITYVOTING_H
...
class WeightedMajorityVoting : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "WeightedMajorityVoting"; };
	static IObject *Create (const ssi_char_t *file) { return new WeightedMajorityVoting (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Weighted Majority Voting"; };
...
protected:
	...
	WeightedMajorityVoting (const ssi_char_t *file = 0);
	virtual ~WeightedMajorityVoting ();
