...
#ifndef SSI_MODEL_MINRULE_H
#define SSI_MODEL_MINRULE_H
...
class MinRule : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "MinRule"; };
	static IObject *Create (const ssi_char_t *file) { return new MinRule (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Min Rule"; };
...
protected:
	...
	MinRule (const ssi_char_t *file = 0);
	virtual ~MinRule ();
