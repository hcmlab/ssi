...
#ifndef SSI_MODEL_PRODUCTRULE_H
#define SSI_MODEL_PRODUCTRULE_H
...
class ProductRule : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "ProductRule"; };
	static IObject *Create (const ssi_char_t *file) { return new ProductRule (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Product Rule"; };
...
protected:
	...
	ProductRule (const ssi_char_t *file = 0);
	virtual ~ProductRule ();
