...
#ifndef SSI_MODEL_BORDACOUNT_H
#define SSI_MODEL_BORDACOUNT_H
...
class BordaCount : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples){ return samples.getStreamSize();};

	static const ssi_char_t *GetCreateName () { return "BordaCount"; };
	static IObject *Create (const ssi_char_t *file) { return new BordaCount (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: Borda Count"; };
	...
	void sort (ssi_size_t nSize, ssi_real_t *anArray, ssi_size_t *order);
	void real_swap (ssi_real_t &x, ssi_real_t &y);
	void size_swap (ssi_size_t &x, ssi_size_t &y);
...
protected:
	...
	BordaCount (const ssi_char_t *file = 0);
	virtual ~BordaCount ();
	...
	ssi_size_t _n_models_actual;
	ssi_size_t *_order;
	ssi_real_t **_decision_profile;
