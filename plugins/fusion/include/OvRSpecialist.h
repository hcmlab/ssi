...
#ifndef SSI_MODEL_OVRSPECIALIST_H
#define SSI_MODEL_OVRSPECIALIST_H
...
class OvRSpecialist : public IFusion {

public:

	ssi_size_t getModelNumber(ISamples &samples);

	static const ssi_char_t *GetCreateName () { return "OvRSpecialist"; };
	static IObject *Create (const ssi_char_t *file) { return new OvRSpecialist (file); };
	
	IOptions *getOptions () { return 0; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Fusion: OvRSpecialist"; };
...
protected:
	...
	OvRSpecialist (const ssi_char_t *file = 0);
	virtual ~OvRSpecialist ();
