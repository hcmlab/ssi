...
namespace ssi {
	...
	ssi_char_t *SumRule::ssi_log_name = "sumrule___";

SumRule::SumRule (const ssi_char_t *file) 
	:	_file (0),
		_n_classes (0),
		_n_streams (0),
		_n_models (0),
...
bool SumRule::train (ssi_size_t n_models,
	IModel **models,
	ISamples &samples) {
	...
	if (samples.getStreamSize () < n_models) {
		ssi_wrn ("#models (%u) > #streams (%u)", n_models, samples.getStreamSize ());
		return false;
	}
...
bool SumRule::forward (ssi_size_t n_models,
	IModel **models,
	...
	if (_n_streams < n_models) {
		ssi_wrn ("#models (%u) > #streams (%u)", n_models, _n_streams);
		return false;
	}
...
void SumRule::print_DP(ssi_real_t ** decision_profile, ssi_real_t* probs){
...
bool SumRule::save (const ssi_char_t *filepath) {
...
bool SumRule::load (const ssi_char_t *filepath) {
