...
namespace ssi {
	...
	ssi_char_t *WeightedAverage::ssi_log_name = "w_avg____";

WeightedAverage::WeightedAverage (const ssi_char_t *file) 
	:	_weights (0),
		_file (0),
		_n_classes (0),
		_n_streams (0),
...
bool WeightedAverage::train (ssi_size_t n_models,
	IModel **models,
	ISamples &samples) {
...
bool WeightedAverage::forward (ssi_size_t n_models,
	IModel **models,
	...
	if(found_data){
		...
		//fill probs with weighted votes
		...
		for(ssi_size_t n_model = 0; n_model < _n_models_actual; n_model++){
			...
			case ('a'):
				for (ssi_size_t num_probs = 0; num_probs < n_probs; num_probs++){
					probs[num_probs] = probs[num_probs]+( decision_profile[n_model][num_probs] * _weights[models_actual[n_model]][_n_classes] );
					probs[num_probs] = probs[num_probs] / ssi_cast(ssi_real_t, _n_models_actual);
				}
				
				break;
			case ('c'):
				for (ssi_size_t num_probs = 0; num_probs < n_probs; num_probs++){
					probs[num_probs] = probs[num_probs]+( decision_profile[n_model][num_probs] * _weights[models_actual[n_model]][num_probs] );
					probs[num_probs] = probs[num_probs] / ssi_cast(ssi_real_t, _n_models_actual);
				}
...
void WeightedAverage::print_DP(ssi_real_t ** decision_profile, ssi_real_t* probs){
...
bool WeightedAverage::save (const ssi_char_t *filepath) {
...
bool WeightedAverage::load (const ssi_char_t *filepath) {
