...
namespace ssi {
	...
	ssi_char_t *BordaCount::ssi_log_name = "borda_____";

BordaCount::BordaCount (const ssi_char_t *file) 
	:	_file (0),
		_n_classes (0),
		_n_streams (0),
		_n_models (0),
...
bool BordaCount::train (ssi_size_t n_models,
	IModel **models,
	ISamples &samples) {
...
bool BordaCount::forward (ssi_size_t n_models,
	IModel **models,
	...
	if(found_data){
		...
		ssi_size_t *order = new ssi_size_t[_n_classes];
		...
		for (ssi_size_t n_model = 0; n_model < _n_models_actual; n_model++) {
			...
			sort (_n_classes, decision_profile[n_model], order);

			for (ssi_size_t num_probs = n_probs; num_probs > 0; num_probs--){
				probs[order[num_probs-1]] = probs[order[num_probs-1]] + num_probs - 1;
			}
		}
...
void BordaCount::sort (ssi_size_t nSize, ssi_real_t *anArray, ssi_size_t *order) {
...
SSI_INLINE void BordaCount::real_swap (ssi_real_t &x, ssi_real_t &y) {
	ssi_real_t z = x;
	x = y;
	y = z;
}

SSI_INLINE void BordaCount::size_swap (ssi_size_t &x, ssi_size_t &y) {
	ssi_size_t z = x;
	x = y;
	y = z;
}

void BordaCount::print_DP(ssi_real_t ** decision_profile, ssi_real_t* probs){
...
bool BordaCount::save (const ssi_char_t *filepath) {
...
bool BordaCount::load (const ssi_char_t *filepath) {
