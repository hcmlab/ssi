...
namespace ssi {
	...
	ssi_char_t *MaxRule::ssi_log_name = "maxrule___";

MaxRule::MaxRule (const ssi_char_t *file) 
	:	_file (0),
		_n_classes (0),
		_n_streams (0),
		_n_models (0),
...
bool MaxRule::train (ssi_size_t n_models,
	IModel **models,
	ISamples &samples) {
...
bool MaxRule::forward (ssi_size_t n_models,
	IModel **models,
	...
	for (ssi_size_t n_model = 0; n_model < n_models; n_model++) {
		decision_profile[n_model] = new ssi_real_t[n_probs];		
	}
...
void MaxRule::print_DP(ssi_real_t **decision_profile, ssi_real_t *probs){
...
bool MaxRule::save (const ssi_char_t *filepath) {
...
bool MaxRule::load (const ssi_char_t *filepath) {
