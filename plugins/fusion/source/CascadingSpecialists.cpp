...
namespace ssi {
	...
	ssi_char_t *CascadingSpecialists::ssi_log_name = "cascspec__";

CascadingSpecialists::CascadingSpecialists (const ssi_char_t *file) 
	:_weights (0),
	_order (0),
	_winner (0),
	_filler (0),
...
CascadingSpecialists::~CascadingSpecialists () { 
	release();
}

void CascadingSpecialists::release ()
{
	if (_weights) {
...
bool CascadingSpecialists::train (ssi_size_t n_models,
	IModel **models,
	ISamples &samples) {
...
void CascadingSpecialists::sort (ssi_size_t nSize, ssi_real_t *anArray, ssi_size_t *order) {
...
SSI_INLINE void CascadingSpecialists::real_swap (ssi_real_t &x, ssi_real_t &y) {
	ssi_real_t z = x;
	x = y;
	y = z;
}

SSI_INLINE void CascadingSpecialists::size_swap (ssi_size_t &x, ssi_size_t &y) {
	ssi_size_t z = x;
	x = y;
	y = z;
}

bool CascadingSpecialists::forward (ssi_size_t n_models,
	IModel **models,
...
bool CascadingSpecialists::save (const ssi_char_t *filepath) {
...
bool CascadingSpecialists::load (const ssi_char_t *filepath) {
