...
#include <ssiocv.h>
#include "../include/facecrop.h"

namespace ssi {

	Facecrop::Facecrop(const ssi_char_t *file)
		: _stride_in(0),
		_stride_out(0),
		_listener(0),
		_face_detector(0),
	...
	bool Facecrop::setEventListener(IEventListener *listener) {
	...
	void Facecrop::setFormatIn(ssi_video_params_t format) {
	...
	void Facecrop::setFormatOut(ssi_video_params_t format) {
	...
	void Facecrop::setFormat(ssi_video_params_t format_in) {

		Facecrop::setFormatIn(format_in);
		Facecrop::setFormatOut(format_in);
	}

	void Facecrop::transform_enter(ssi_stream_t &stream_in,
	...
	void Facecrop::transform(ITransformer::info info,
	...
	void Facecrop::transform_flush(ssi_stream_t &stream_in,
		ssi_stream_t &stream_out,
		ssi_size_t xtra_stream_in_num,
		ssi_stream_t xtra_stream_in[]) {
