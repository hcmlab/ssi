...
namespace ssi {
	...
	public:
		...
		static const ssi_char_t *GetCreateName() { return "Facecrop"; };
		static IObject *Create(const ssi_char_t *file) { return new Facecrop(file); };
		~Facecrop();
		Facecrop::Options *getOptions() { return &_options; };
		const ssi_char_t *getName() { return GetCreateName(); };
		const ssi_char_t *getInfo() { return "detects body features in a video stream"; };
	...
	protected:
		...
		Facecrop(const ssi_char_t *file = 0);
		Facecrop::Options _options;
		ssi_char_t *_file;
