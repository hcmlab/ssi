...
namespace ssi {
	Openness::Openness(const ssi_char_t *file)
		: _file(0) {
		if (file) {
	...
	void Openness::transform(ITransformer::info info,
		ssi_stream_t &stream_in,
		ssi_stream_t &stream_out,
		ssi_size_t xtra_stream_in_num,
		ssi_stream_t xtra_stream_in[]) {
		...
		{
			leftx = leftx + ss[i][SSI_SKELETON_JOINT::LEFT_HAND][SSI_SKELETON_JOINT_VALUE::POS_X];
			lefty = lefty + ss[i][SSI_SKELETON_JOINT::LEFT_HAND][SSI_SKELETON_JOINT_VALUE::POS_Y];
			rightx = rightx + ss[i][SSI_SKELETON_JOINT::RIGHT_HAND][SSI_SKELETON_JOINT_VALUE::POS_X];
			righty = righty + ss[i][SSI_SKELETON_JOINT::RIGHT_HAND][SSI_SKELETON_JOINT_VALUE::POS_Y];
		}
		...
		ssi_real_t result = sqrt((leftx - rightx) * (leftx - rightx) + (lefty - righty) * (lefty - righty));
		if (rightx < leftx)
		{
			result = -result;
		}
		if (_options.normalize == true)
		{
	...
	ssi_real_t Openness::normalizevalue(ssi_real_t value, ssi_real_t min, ssi_real_t max)
	{
		ssi_real_t result = (ssi_real_t)((value - min)*(1 / (max - min)));

		if (result < min) return 5 * result;
