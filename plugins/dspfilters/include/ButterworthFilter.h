...
#ifndef SSI_SIGNAL_BUTTERWORTHFILTER_H
#define SSI_SIGNAL_BUTTERWORTHFILTER_H
...
class ButterworthFilter : public IDspFilter {
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ButtworthFilter"; };
	static IObject *Create (const ssi_char_t *file) { return new ButterworthFilter (file); };
	virtual ~ButterworthFilter ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "provides butterworth filtering"; };

	Dsp::Filter *createFilter (DspFiltersTools::FilterProperty &prop);

protected:

	ButterworthFilter (const ssi_char_t *file = 0);
	ButterworthFilter::Options _options;
	ssi_char_t *_file;
