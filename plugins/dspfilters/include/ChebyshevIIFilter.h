...
#ifndef SSI_SIGNAL_CHEBYSHEVIIFILTER_H
#define SSI_SIGNAL_CHEBYSHEVIIFILTER_H
...
class ChebyshevIIFilter : public IDspFilter {
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ChebyshevIIFilter"; };
	static IObject *Create (const ssi_char_t *file) { return new ChebyshevIIFilter (file); };
	virtual ~ChebyshevIIFilter ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "provides chebyshev II filtering"; };

	Dsp::Filter *createFilter (DspFiltersTools::FilterProperty &prop);

protected:

	ChebyshevIIFilter (const ssi_char_t *file = 0);
	ChebyshevIIFilter::Options _options;
	ssi_char_t *_file;
