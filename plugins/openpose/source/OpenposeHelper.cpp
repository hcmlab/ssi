...
// Debugging/Other
DEFINE_int32(logging_level, 255, "The logging level. Integer in the range [0, 255]. 0 will output any log() message, while"
...
DEFINE_string(face_net_resolution, "320x320", "Multiples of 16 and squared. Analogous to `net_resolution` but applied to the face keypoint"
...
DEFINE_string(write_coco_json, "", "Full file path to write people pose data with JSON BODY validation format.");
DEFINE_string(write_coco_foot_json, "", "Full file path to write people foot pose data with JSON BODY validation format.");
...
namespace ssi {
	...
	OpenposeHelper::OpenposeHelper(std::string modelFolder, std::string net_resolution, std::string hand_net_resolution, std::string face_net_resolution, int numberOfMaxPeople, bool hand, bool face)
	{
		FLAGS_model_folder = modelFolder;

		FLAGS_net_resolution = net_resolution;
		FLAGS_hand_net_resolution = hand_net_resolution;
		FLAGS_face_net_resolution = face_net_resolution;
		FLAGS_number_people_max = numberOfMaxPeople;
		FLAGS_hand = hand;
		FLAGS_face = face;
		initializeOpenposeHelper();
	}
	...
	void OpenposeHelper::initializeOpenposeHelper()
	{
		// logging_level
		...
		// Configure wrapper
		//pose, face and hand active
		if (FLAGS_face && FLAGS_hand) {
			opWrapper.configure(wrapperStructPose, wrapperStructFace, wrapperStructHand, wrapperStructExtra, op::WrapperStructInput{},
				op::WrapperStructOutput{}); 
		}

		//only pose and face active
		else if (FLAGS_face) {
			opWrapper.configure(wrapperStructPose, wrapperStructFace, op::WrapperStructHand{}, wrapperStructExtra, op::WrapperStructInput{},
				op::WrapperStructOutput{});
		}

		//only pose and hand active
		else if (FLAGS_hand) {
			opWrapper.configure(wrapperStructPose, op::WrapperStructFace{}, wrapperStructHand, wrapperStructExtra, op::WrapperStructInput{},
				op::WrapperStructOutput{});
		}
		//only pose active
		else {
			opWrapper.configure(wrapperStructPose, op::WrapperStructFace{}, op::WrapperStructHand{}, wrapperStructExtra, op::WrapperStructInput{},
				op::WrapperStructOutput{});
		}
	...
	void OpenposeHelper::getKeyPoints(cv::Mat inputImage, float * out) {
		
		// Create new datum
		auto datumToProcess = std::make_shared<std::vector<op::Datum>>();
		datumToProcess->emplace_back();
		auto& datum = datumToProcess->at(0);
		...
		const auto& handLeftKeypoints = datumProcessed->at(0).handKeypoints[0];
		const auto& handRightKeypoints = datumProcessed->at(0).handKeypoints[1];

		//if(faceKeypoints.empty())
		//	ssi_print("face empty ");

		auto numberPeopleDetected = poseKeypoints.getSize(0);
		const auto numberBodyParts = poseKeypoints.getSize(1);
		numberPeopleDetected = (numberPeopleDetected > FLAGS_number_people_max) ? FLAGS_number_people_max : numberPeopleDetected;

		//ssi_print("openpose : people detected %d : \n", numberPeopleDetected);
		...
		if (!poseKeypoints.empty()) {
			...
			for (int person = 0; person < numberPeopleDetected; ++person) {
				for (int part = 0; part < numberBodyParts; ++part) {
					...
					//x
					out[count++] = poseKeypoints[baseIndex];
					//y
					out[count++] = poseKeypoints[baseIndex + 1];
					//Score
					out[count++] = poseKeypoints[baseIndex + 2];
				}
			}
		}
	
		if (!faceKeypoints.empty()) {
			int count = FLAGS_number_people_max * OpenposeModelInfos::SKELETON_JOINT_BODY::NUM * OpenposeModelInfos::SKELETON_JOINT_VALUE::NUM;
			for (int person = 0; person < numberPeopleDetected; ++person) {
				for (int part = 0; part < faceKeypoints.getSize(1); ++part) {
					const auto baseIndex = faceKeypoints.getSize(2)*(person * 	OpenposeModelInfos::SKELETON_FACE_MODEL::PARTS + part);
					//x
					out[count++] = faceKeypoints[baseIndex];
					//y
					out[count++] = faceKeypoints[baseIndex + 1];
					//Score
					out[count++] = faceKeypoints[baseIndex + 2];
				}
			}

			//ssi_print("facesize: %d \n", faceKeypoints.getSize());

		}

		if (!handLeftKeypoints.empty()) {
			int count = FLAGS_number_people_max * OpenposeModelInfos::SKELETON_JOINT_BODY::NUM * OpenposeModelInfos::SKELETON_JOINT_VALUE::NUM;
			if (FLAGS_face) count += FLAGS_number_people_max * OpenposeModelInfos::SKELETON_FACE_MODEL::PARTS  * OpenposeModelInfos::SKELETON_FACE_MODEL::DIM; //face

			for (int person = 0; person < numberPeopleDetected; ++person) {
				for (int part = 0; part < OpenposeModelInfos::SKELETON_HAND_MODEL::PARTS; ++part) {
					const auto baseIndex = handLeftKeypoints.getSize(2)*(person *  OpenposeModelInfos::SKELETON_HAND_MODEL::PARTS + part);
					//x
					out[count++] = handLeftKeypoints[baseIndex];
					//y
					out[count++] = handLeftKeypoints[baseIndex + 1];
					//Score
					out[count++] = handLeftKeypoints[baseIndex + 2];
				}
			}

		}

		if (!handRightKeypoints.empty()) {
			int count = FLAGS_number_people_max * OpenposeModelInfos::SKELETON_JOINT_BODY::NUM * OpenposeModelInfos::SKELETON_JOINT_VALUE::NUM //pose
				+ FLAGS_number_people_max * OpenposeModelInfos::SKELETON_HAND_MODEL::PARTS *  OpenposeModelInfos::SKELETON_HAND_MODEL::DIM; //lefthand
			if (FLAGS_face) count += FLAGS_number_people_max * OpenposeModelInfos::SKELETON_FACE_MODEL::PARTS  * OpenposeModelInfos::SKELETON_FACE_MODEL::DIM;  //face

			for (int person = 0; person < numberPeopleDetected; ++person) {
				for (int part = 0; part < OpenposeModelInfos::SKELETON_HAND_MODEL::PARTS; ++part) {
					const auto baseIndex = handRightKeypoints.getSize(2)*(person * OpenposeModelInfos::SKELETON_HAND_MODEL::PARTS + part);
					//x
					out[count++] = handRightKeypoints[baseIndex];
					//y
					out[count++] = handRightKeypoints[baseIndex + 1];
					//Score
					out[count++] = handRightKeypoints[baseIndex + 2];
				}
			}
