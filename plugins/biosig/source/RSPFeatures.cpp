...
ssi_char_t *RSPFeatures::ssi_log_name = "RSPFeatures__";

RSPFeatures::RSPFeatures (const ssi_char_t *file)
	: _file (0),
...
void RSPFeatures::transform_enter(ssi_stream_t &stream_in, ssi_stream_t &stream_out, ssi_size_t xtra_stream_in_num,	ssi_stream_t xtra_stream_in[]) {
...
void RSPFeatures::transform( ITransformer::info info, ssi_stream_t &stream_in, ssi_stream_t &stream_out, ssi_size_t xtra_stream_in_num, ssi_stream_t xtra_stream_in[]) {
...
ssi_real_t RSPFeatures::getVolume(ssi_real_t* in, ssi_size_t from, ssi_size_t to){
	ssi_real_t sum = 0;
	for (int i = from; i < to; i++)	sum += in[i];
	return fabsf(sum);
}

ssi_real_t RSPFeatures::getFlowRate(ssi_real_t* in, ssi_size_t from, ssi_size_t to){
	ssi_real_t sum = 0;
...
void RSPFeatures::transform_flush(ssi_stream_t &stream_in,
...
void RSPFeatures::printFeatures(){
	int af;
	af = 0;
	ssi_print("\n[RSPFeatures] Activated Features:\n");
