...
ssi_char_t *QRSHeartRateMean::ssi_log_name = "hrmean___";

QRSHeartRateMean::QRSHeartRateMean (const ssi_char_t *file)
	: _file (0),
	_listener (0),
	_src_sr (0),
	_sample_counter (0),
...
QRSHeartRateMean::~QRSHeartRateMean () {
...
bool QRSHeartRateMean::setEventListener (IEventListener *listener) {
...
void QRSHeartRateMean::consume_enter (ssi_size_t stream_in_num,
...
void QRSHeartRateMean::consume (IConsumer::info consume_info,
	...
	for(ssi_size_t nsamp = 0; nsamp < n_samples; nsamp++){
		...
		if(_sample_counter >= _win_samples){
			...
			if(firstrun){
				...
				ssi_real_t current_sample_time = _sample_counter * time_per_sample;

				
				mean = _sample_sum / _sample_counter;
				if(_last_mean != 0.0f){
					p_change = _options.amplifier * (mean / _last_mean - 1.0f);
				}else{
					p_change = 1.0f;
				}
				...
				if (_listener) {
					...
					_event.dur = ssi_size_t(current_sample_time);
...
void QRSHeartRateMean::consume_flush (ssi_size_t stream_in_num,
	ssi_stream_t stream_in[]) {
