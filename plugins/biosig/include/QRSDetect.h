...
#ifndef SSI_FILTER_QRSDETECT_H
#define SSI_FILTER_QRSDETECT_H
...
class QRSDetect : public IFilter {
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "QRSDetect"; };
	static IObject *Create (const ssi_char_t *file) { return new QRSDetect (file); };
	~QRSDetect ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Applies qrs complex detection to an ecg signal. Frames with value 1 mark an R-spike"; };
...
protected:
	...
	QRSDetect (const ssi_char_t *file = 0);
	QRSDetect::Options _options;
	ssi_char_t *_file;
