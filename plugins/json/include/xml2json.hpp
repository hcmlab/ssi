...
static bool xml2json_has_digits_only(const char * input, bool *hasDecimal)
{
    ...
    *hasDecimal = false;
    ...
    while (*runPtr != '\0')
    {
        ...
        else if (!isdigit((unsigned char)*runPtr))
        {
            return false;
        }
        runPtr++;
...
void xml2json_add_attributes(rapidxml::xml_node<> *xmlnode, rapidjson::Value &jsvalue, rapidjson::Document::AllocatorType& allocator, xml2json_params &params)
{
    rapidxml::xml_attribute<> *myattr;
    for(myattr = xmlnode->first_attribute(); myattr; myattr = myattr->next_attribute())
    {
        rapidjson::Value jn, jv;
        jn.SetString((std::string(params.attribute_prefix ? xml2json_attribute_name_prefix : "") + myattr->name()).c_str(), allocator);

        if (params.numeric_support == false)
        {
...
void xml2json_traverse_node(rapidxml::xml_node<> *xmlnode, rapidjson::Value &jsvalue, rapidjson::Document::AllocatorType& allocator, xml2json_params &params)
{
    ...
    else if(xmlnode->type() == rapidxml::node_element)
    {
        if(xmlnode->first_attribute())
        {
            if(xmlnode->first_node() && xmlnode->first_node()->type() == rapidxml::node_data && count_children(xmlnode) == 1)
            {
                ...
                rapidjson::Value jn, jv;
                jn.SetString(params.text_prefix ? xml2json_text_additional_name : "", allocator);
                jv.SetString(xmlnode->first_node()->value(), allocator);
                jsvalue.AddMember(jn, jv, allocator);
                xml2json_add_attributes(xmlnode, jsvalue, allocator, params);
                return;
            }
            else
            {
        ...
        if(xmlnode->first_node())
        {
            ...
            for(xmlnode_chd = xmlnode->first_node(); xmlnode_chd; xmlnode_chd = xmlnode_chd->next_sibling())
            {
                ...
                {
                    current_name = params.text_prefix ? xml2json_text_additional_name : "";
                    name_count[current_name]++;
                    jv.SetString(params.text_prefix ? xml2json_text_additional_name : "", allocator);
                ...
                xml2json_traverse_node(xmlnode_chd, jsvalue_chd, allocator, params);
...
std::string xml2json(const char *xml_str, xml2json_params &params)
{
