    ...
    if(BGHS[SI].EXTENDLASTCOLUMN)
	 ...
	 while(r<=BGHS[SI].bottomvisiblerow)
	 {
		 ...
		 else
		 {
		  ...
		  HPEN hpen,holdpen;
		  iProtection = (int)SendMessage(hWnd, BGM_GETPROTECTION, (WPARAM)&BGcell, 0);
		 ...
		 iDataType = (int)SendMessage(hWnd, BGM_GETTYPE, (WPARAM)&BGcell, 0);
		 ...
		 if(iDataType == 1)//ALPHA
		 {
          if(BGHS[SI].ELLIPSIS)
              ...
              {
				  DrawTextEx(gdc, buffer, -1, &rect, DT_CENTER | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX, NULL);
              }
		 }
...
char GetASCII(WPARAM wParam, LPARAM lParam)
    {
     ...
     result=(int) ToAscii((UINT) wParam,(lParam >> 16) && 0xff,keys,&dwReturnedValue,0);
...
void DisplayEditString(HWND hWnd,int SI,char* tstring)
    {
       ...
       rt.right -= 5;
	   DrawText(cdc, BGHS[SI].editstringdisplay, -1, &rt, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
       rt.right +=5;
       ShowCaret(hWnd);
...
LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
        ...
        case BGM_SETCELLDATA:
			  ...
			  FindResult = (int) SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, 0, (LPARAM)buffer);
     ...
     if(lbcount < 12)
         {
          ...
          FindResult = (int) SendMessage(lbhWnd,LB_FINDSTRING,-1,(LPARAM) searchtext);
