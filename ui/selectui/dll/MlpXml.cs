...
namespace ssi
{
    ...
    public delegate void ProcessUpdateDelegate(double start, double duration, string name, bool store, bool change);
    ...
    public class MlpXmlTrain
    {
        ...
        public static void Start()
        {
            ...
            process.StartInfo.CreateNoWindow = (eval > 3) ? false : true;
            //process.OutputDataReceived += OutputStreamHandler;
            StringBuilder arguments = new StringBuilder();
            arguments.Append (@"--train -eval " + eval + " -kfolds " + kfolds + " -remote" + " -reps " + reps + " -fps " + fps );
            if (log != null && log != "")
            {
                ...
                if (msg != null)
                {
                    if (processUpdateEvent != null)
                    {
                        ...
                        else
                        {
                            ...
                            string label = words[2].Substring(0, words[2].Length);
                            int store = Int32.Parse(words[3], System.Globalization.CultureInfo.InvariantCulture);
                            int change = Int32.Parse(words[4], System.Globalization.CultureInfo.InvariantCulture);
                            processUpdateEvent(start, duration, label, store != 0, change != 0);
                        }
                    }
                    else
                    {
