...
public:
	...
	static void LoadAnnotation (old::Annotation &anno, 
		File &file, char* tier_id = "");
	static void LoadAnnotation (old::Annotation &anno,
		const ssi_char_t *file_name, char* tier_id = "");
	static void SaveAnnotation (old::Annotation &anno, 
		File &file);
	static void SaveAnnotation (old::Annotation &anno, 
		const ssi_char_t *file_name);
	static void ConvertToContinuousAnnotation (old::Annotation &from,
		old::Annotation &to,
	...
	static void CreateDynamicTestSamples (SampleList &samples, 
		...
		ssi_size_t num_max,
		const ssi_char_t* user = "user");
	static void CreateMissingData(SampleList &samples, ssi_real_t prob);
	static void CreateTestSamplesRegression(SampleList &samples,
	...
	static void PrintInfo (ISamples &samples, FILE *file = 0);
	...
	static void PlotSamples(ISamples & samples, const ssi_char_t * name, ssi_rect_t pos, ssi_size_t circle_size = 10, ssi_rgb_t bg_color = ssi_rgb(0, 0, 0), ssi_rgb_t circle_color = ssi_rgb(0, 0, 0), ssi_size_t n_classes_colors = 0, ssi_rgb_t classes_colors[] = 0);
	static void PlotSamplesRegression(ISamples & samples, const ssi_char_t * name, ssi_rect_t pos, ssi_rgb_t color = ssi_rgb(128, 0, 0), ssi_size_t circle_size = 10, ssi_rgb_t bg_color = ssi_rgb(0, 0, 0), ssi_rgb_t circle_color = ssi_rgb(0, 0, 0));
