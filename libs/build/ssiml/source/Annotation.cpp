...
namespace ssi {
	...
	bool Annotation::setDiscreteScheme(const ssi_char_t *scheme_name,
		std::map<String, ssi_size_t> &classes, bool IsExternalTraining)
	{
		release();
	...
	bool Annotation::addClass(const ssi_char_t *name, bool externalTraining)
	{
		if (!_scheme || _scheme->type != SSI_SCHEME_TYPE::DISCRETE)
		{
			ssi_wrn("not a discrete scheme");
		...
		StringList names;
		names.add(name);
		return addClass(names, externalTraining);
	}

	bool Annotation::addClass(const ssi_char_t *name, ssi_int_t &id, bool externalTraining)
	{
		if (addClass(name, externalTraining))
		{
			getClassId(name, id);
			return true;
		}
	...
	bool Annotation::addClass(ssi_int_t class_id, const ssi_char_t *name, bool externalTraining)
	{
		if (hasClassId(class_id))
		{
			ssi_int_t id;
	...
	bool Annotation::addClass(StringList names, bool externalTraining)
	{
		if (!_scheme || _scheme->type != SSI_SCHEME_TYPE::DISCRETE)
		{
			ssi_wrn("not a discrete scheme");
