...
// Interface functions
//
model* train_model(const problem *prob, const parameter *param)
{
...
model* warm_start_train(const problem *prob, const parameter *param, const model *wsmodel)
{
	...
	else
	{
        ...
        {
            model_->nr_class = wsmodel->nr_class;
            model_->label = Malloc(int,nr_class);
            for(i=0;i<nr_class;i++)
                model_->label[i] = wsmodel->label[i];
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, double *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = train_model(&subprob,param);
		for(j=begin;j<end;j++)
	...
	*best_rate = 0;
	...
	while(param1.C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *submodel = train_model(&subprob[i],&param1);
