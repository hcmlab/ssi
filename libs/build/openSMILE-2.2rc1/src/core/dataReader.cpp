...
int cDataReader::myFinaliseInstance()
{
  ...
  for (i=0; i<nLevels; i++) {
    ...
    if (c!=NULL) {
      ...
      if (lT == -1.0) lT = c->T;
      else if ((c->T != lT)&&(!forceAsyncMerge)) {
        SMILE_IERR(1,"frame period mismatch among input levels! '%s':%f <> '%s':%f",dm->getLevelName(level[i]),c->T,dm->getLevelName(level[0]),lT);
        return 0;
      }
      if (ldtype==-1) ldtype = c->type;
      else if (ldtype != c->type) {
        SMILE_IERR(1,"dataType mismatch among input levels (conversion is not yet supported!) '%s':%i <> '%s':%i",dm->getLevelName(level[i]),c->type,dm->getLevelName(level[0]),ldtype);
        return 0;
      }
